...
export const PERMISSIONS = ['read','download','write','review','publish','share','manage'];
...
export function invalidateAccessCache(state){cache.delete(state);}
...
export function scopeFor(state,kind,id){const i=index(state);if(kind==='workspace')return{scope:'workspace',resourceId:state.id};const row=i[kind+'s']?.get(id);if(!row)fail('Resource not found.','NOT_FOUND');return{scope:kind,resourceId:row.id};}
export function createAccessContext(state,userOrId){
...
export function canAccess(state,user,permission,scope='workspace',id=state.id){return createAccessContext(state,user).can(permission,scope,id);}
export function requireAccess(state,user,permission,scope,id){if(!canAccess(state,user,permission,scope,id))fail();}
export function normalizePolicy(state,input){
...
export function validateAccessState(state){
...
export function notificationVisible(state,notice,userId){
...
export function authorizeCommand(state,command,userId){
...
export function projectWorkspace(state,userId){
