...
export const COMPARISON_LIMITS=Object.freeze({textBytes:2*1024*1024,fileBytes:50*1024*1024,lines:20000,editDistance:1024,work:4000000});
export class ComparisonError extends Error {constructor(message,code='VALIDATION'){super(message);this.name='ComparisonError';this.code=code;}}
...
export function comparisonKind(name){const ext=String(name).split('.').at(-1).toLowerCase();return ['svg','png','jpg','jpeg','webp','gif','bmp'].includes(ext)?'image':['txt','md','csv','json','xml','obj','mtl','log','js','css','html','yaml','yml','dxf','ini','sql'].includes(ext)?'text':'binary';}
export async function readVerifiedRevision(version,getBlob,{signal}={}){
...
export async function decodeComparisonText(blob){
...
export function newlineCounts(text){return {crlf:(text.match(/\r\n/g)||[]).length,lf:(text.match(/(?<!\r)\n/g)||[]).length,cr:(text.match(/\r(?!\n)/g)||[]).length};}
...
export function compareLines(leftText,rightText,{ignoreWhitespace=false,ignoreLineEndings=true}={}){
...
export function comparisonReport({document,baseline,target,result,decoding=null,warning=''}){
